...
stages 主要是用来定义某一个流水线阶段的。其中还包含 stage、steps、script。其中 stage 代表多个阶段；steps 代表的是多个步骤以及 script 代表具体执行的脚本。
...
pipeline {
    ...
    parameters {
        listGitBranches 
            ...
            remoteURL: 'git@gitlab.abck8s.com:xiaoyangaijishu/html.git', 
            selectedValue: 'NONE', 
            sortMode: 'NONE', 
            tagFilter: '*', 
            type: 'PT_BRANCH'
    ...
    post {
      always {
        emailext body: '123', subject: '456', to: 'chenyangqit@163.com'
      }
    }
}
```
